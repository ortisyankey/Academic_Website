...
# Hierarchichal Bottom-Up Modelling Tutorial

## Introduction

The purpose of this tutorial is to introduce readers to the hierarchical bottom-up population modelling. The series of tutorials is not an introduction to statistics. For these tutorials, it would be good to be familiar with some statistical
concepts, and for that purpose, we suggest useful resources below that users can read:
...
-   **Markov chain Monte Carlo (MCMC)**, a simulation-based method for model
    estimation:

    -   [Markov chains explained visually](https://setosa.io/ev/markov-chains/) by Victor Powell
    -   [Metropolis-Hastings Monte Carlo](https://www.bayesrulesbook.com/chapter-7.html), a
...
Population modelling for this tutorial is done using the **`rstan package`** in R. Further information on stan can be 
found below
...
> **Note** For a more interactive version of the tutorial, including knowledge quizzes, please go to: https://wpgp.github.io/bottom-up-tutorial/.


## Part I: How to think about population as a Bayesian

The first part of this tutorial is to introduce readers to the basics of Bayesian modeling and to apply Bayesian principles to bottom-up gridded population estimation. We cover how to transition from frequentist modelling to Bayesian thinking and apply Bayesian principles to fit a poisson model for population count. We then build on this model to fit a poisson-lognormal model, which is used for bottom-up population modelling. In summary, the goals of this part are to:
...
### Set-up
This model will be implemented in the R programming language [@r2020r]. Our first step is to setup the R environment so that it contains the R packages and data that we will need. First, download the most recent versions of the following softwares:
...
We will also need the following r packages to be installed if you have not installed them already. 
...
Once installed, you must load the installed package into your R environment.
...
After loading the packages in R, we need to setup stan for compilation of the model by specifying the number of cores to use and increase the speed running time for fitting a model
...
*The rstan package allows one to conveniently fit Stan models from R [@r2020r] and access the output, including posterior
inferences and intermediate quantities such as evaluations of the log posterior density and its gradients.*
...
$$Y \sim Normal(mean=\mu, sd=\sigma)$$ $$ \mu=\alpha + \beta X$$

This format is more flexible when we start working with **non-normal error structures** and **custom modelling components**. This linear regression can be represented using a directed acyclic graph (DAG) that helps to picture the relationships between model parameters and input data:
...
dagify(
    ...
    mutate(color=c('data','data',  'parameter','parameter',  'parameter','parameter')) %>% 
...
In the DAG above, squares represent data and circles represent parameters. Directions of arrows indicate dependence.
...
The bayesrules book has a very good [chapter](https://www.bayesrulesbook.com/chapter-4.html#) on the interaction between priors and data. The `stan` team put together interesting [guidelines](https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations)
to prior setting.
...
We produce our fake data as 1000 draws of a Normal distribution with
mean 5 and standard deviation 50:
...
ggplot(data, aes(x=y))+
  ...
  theme_minimal()+
  geom_vline(xintercept = mean(data$y), color='orange', linewidth=1)+
  annotate('text', x=15, y=30, label=paste0('Observed mean: ', round(mean(data$y),2)), colour='orange', angle=90)+
  geom_segment(x=0,y=1, xend=sd(data$y), yend=1, colour='orange', linewidth=1)+
...
Let's pretend we don't know how the observations, $y$ have been created. 
We want to model $y$ and we see that it has a clear
bell shape. It can thus be **approximated with a Normal distribution**
that is: $$Y \sim Normal(mean=\mu, sd=\sigma)$$
...
dagify(
  ...
  mutate(color=c('parameter', 'data','parameter')) %>% 
  ggplot(aes(x = x, y = y, xend = xend, yend = yend, color=color,shape=color)) +
  geom_dag_point() +
  geom_dag_edges() +
  geom_dag_text(col = "grey20",size=6,  parse=T) +
...
**We need to give some prior to the two parameters** $\mu$ the mean and
$\sigma$ the standard deviation. Let's imagine that we don't have access
to more information on the variable $Y$ than its manifestation in our
dataset.

We can guess that $\mu$ is around zero but without great confidence (see
Figure \@ref(fig:sim-distrib)). In other words, we can assume that $\mu$
can be drawn from a Normal(0,100).
...
ggplot(data)+
  ...
  geom_density(aes(x=prior_mu), colour='#00BFC4', linewidth=1)+
  theme_minimal()+
...
Figure \@ref(fig:prior-mu-distrib) shows what our prior for $\mu$ means:
we think that $\mu$ is likely to be around zero but could be up to 200.
Given that our observed $y$, $\hat{y}$, has no occurrences above 200, it
is unlikely that the mean of $y$ is 200 but we don't exclude the
possibilty. However we do consider that 0 is more likely than 200.

For $\sigma$, we have stricter expectations. Indeed standard deviations
are positive. We thus choose as prior for $\sigma$ a Uniform(0, 200).
...
ggplot(data)+
  geom_histogram(aes(x=y, after_stat(density)), bins=50, fill='grey30')+
  geom_density(aes(x=prior_sigma), colour='#00BFC4', linewidth =1, trim=T)+
  theme_minimal()+
...
Figure \@ref(fig:prior-sigma-distrib) shows that a uniform prior means a
equal probability of $\sigma$ to be 0 as to be 200.
...
To estimate $\mu$ and $\sigma$, we will write **our first model** in
stan.
...
The `stan` software requires us to declare all variables, both parameters and data, with their type (int, real) and size (as indicated with `[n]`). It is possible to incorporate constraints on the variable support, e.g. it is not possible to have a negative $\sigma$ (`real<lower=0> sigma`). More details [here](https://mc-stan.org/docs/2_27/reference-manual/overview-of-data-types.html).

Note: `stan` requires to leave one blank line at the end of the script. We will store the model in a `stan` file called `tutorial1_model.stan`
...
This section of the tutorials will cover actual population modelling using sample data for Nigeria. 
Let's download the data we will be modelling. It belongs to the
[supplementary
...
The data consist of household surveys that collected information on the total population in 1141 clusters in 15 of 37 states in Nigeria during 2016 and 2017. Clusters varied slightly in size, but were all approximately 3 hectares. These clusters were randomly sampled locations whose boundaries were drawn based on high resolution satellite imagery.
The surveys are further described in @leasure2020 and @weber2018. Survey site locations are shown in Figure \@ref(fig:ngamap).
...
To estimate $\lambda$, we will write **our first population model** in
stan.
...
We note that all predictions are very similar. The model includes only one parameter --- lambda --- and the site-level
predictions do not account for any site-level characteristics. Therefore, predictions at each site are drawn from the exact same distribution. Let's see the global picture by plotting the observed vs the predicted population count. A perfect model would see all points on the 1:1 line.
...
A source of overdispersion comes from the size of the clusters. We observed population counts for units with different area, in particular different sizes of settled areas as shown in Figure \@ref(fig:pop-area).

```{r pop-area, class.source = 'fold-hide', fig.cap='Scatterplot contrasting population count with settled area for every cluster'}
#plot overdispersion
...
To **incorporate overdispersion** in our model, we decompose population
count as follows:
...
dagify(
  ...
  tidy_dagitty(seed=3) %>% 
  mutate(color=c('parameter','data','data','parameter','parameter')) %>% 
  ggplot(aes(x = x, y = y, xend = xend, yend = yend, color=color,shape=color)) +
  geom_dag_point() +
  geom_dag_edges() +
...
In the Lognormal, there are two parameters, $\mu$ that represents the median of the population density on the log scale, and $\sigma$ the geometric standard deviation of population density on the log scale. We set up their priors similarly as before and retrieve from the data that the log observed median of the population density is
`r round(log(median(data$pop_density)),2)` and the observed log geometric standard deviation is`r round(log(EnvStats::geoSD(data$pop_density)),2)`.
...
```
Note that we need to use the `vector` format for `area` and `pop_density`.
...
Note that the population density is not an input data of the model, but
an unobserved latent variable that we model through the Lognormal.
...
We see that the chains have mixed well and that the posterior
...
We see for the `population_density` the same estimation pattern as in
...
We use the same metrics based on residuals to assess the goodness-of-fit
of the model.
...
The proportion of observations that fall in their credible intervals
shows a well-behaved model: 95.5% of the observations falls in the 95%
credible interval. On average the model overestimates the population
count by around 60 people, but this statistic varies on average by 337
people.

No variations are included in this model: we built a national model of
population count. It will require further refinements that will be
introduced in the next tutorials.
